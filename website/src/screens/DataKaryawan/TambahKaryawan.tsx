...
const TambahKaryawan: React.FC<Props> = ({
  ...
  // Fetch shifts ketika modal dibuka
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (unitKerjaList && unitKerjaList.length > 0) {
          const headOfficeUnit = unitKerjaList.find((unit: any) => unit.nama_unit === "Head Office")
          const unitId = headOfficeUnit ? headOfficeUnit.id : (unitKerjaList[0]?.id || null)
  ...
  const handleSubmitFixed = async (e: React.FormEvent) => {
    ...
    try {
      if (!formData.unit_kerja || !formData.shift_id) {
        alert('Unit kerja dan shift harus dipilih')
        return
      }
        ...
        <div className="bg-white px-4 py-3 rounded-b-xl border-t border-gray-200">
          <div className="flex flex-col sm:flex-row items-center justify-between space-y-2 sm:space-y-0">
            ...
            <div className="flex space-x-2">
              ...
              >
                <span>💾</span>
                <span>{submitLoading ? 'Menyimpan...' : (loadingShifts ? 'Memuat...' : 'Simpan')}</span>
              </button>
            </div>
          </div>
        </div>
